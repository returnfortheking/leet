...
## ★ 闭包替代 `self.xxx`：递归用嵌套函数 + 局部变量

刷题里 90% 的 DFS / 回溯都该用闭包，不是把临时状态挂到 Solution 类上。

**反例**（OOP 思维污染 Solution 类）：

```python
class Solution:
    def inorderTraversal(self, root):
        self.res = []           # ❌ 把临时状态绑到实例
        self.travel(root)       # ❌ travel 不该是 Solution 的方法
        return self.res

    def travel(self, root):     # 每行都 self.
        if not root: return
...
问题：
- `self.res` **跨调用残留**，必须每次手动重置；忘了会 append 在上次结果后面
- `travel` 是临时工具，硬挂到类上**污染了接口**
- 满屏 `self.` 干扰阅读
...
- `res` 是 `inorderTraversal` 的局部变量，调用结束自动消失
- `dfs` 是嵌套函数，**闭包自动捕获 `res`** —— 不需要 self、不需要参数透传
- `res.append(...)` 是**修改**不是重新赋值，**不需要 `nonlocal`**
...
def maxDepth(self, root):
    ans = 0
    def dfs(node, d):
        nonlocal ans            # ★ 因为 ans = ... 是重新绑定
        if not node: return
        ans = max(ans, d)       # 重新赋值 ← 触发
        dfs(node.left, d + 1)
        dfs(node.right, d + 1)
    dfs(root, 1)
    return ans
...
规则：
- `lst.append(x)` / `d[k] = v` / `s.add(x)` —— **修改容器内容**，不需要 nonlocal
- `x = ...` / `x += 1` / `x -= 1` —— **重新绑定变量**，需要 nonlocal

绕开方法：把可变状态包成单元素 list 或 dict，全程只**修改**不**赋值**：

```python
ans = [0]                       # 用 list 当"可变盒子"
def dfs(node, d):
    if not node: return
    ans[0] = max(ans[0], d)     # 改的是 ans[0]，不是 ans 本身 → 不用 nonlocal
    ...
```

但**直接 `nonlocal` 更清晰**，不要为了省一行写晦涩的盒子。

### 适用场景

| 场景 | 写法 |
|---|---|
| 一次性递归收集结果（数组类）| 闭包 + 局部 `res = []` |
| 递归过程中维护全局计数 / 最值 | 闭包 + 局部变量 + `nonlocal` |
| 真正的实例状态（设计题 LRU / MinStack）| 才用 `self.xxx` |

**自查信号**：写完代码看到 `self.res` / `self.cnt` / `self.max` 这种 helper-state —— 大概率应改闭包。

**TS 对照**：TS 也有闭包，但 `function inner() { ... }` 默认就是闭包；Python 区分"读外层"和"写外层"（后者要 `nonlocal`），TS 不区分。
