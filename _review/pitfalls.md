...
**症状**：返回的子串/子数组少了或多了一个元素；常见情境是"我有下标 l 和 r，想拿 [l..r] 这一段"，结果少拿了 r。
**根因**：Python 的切片 `s[a:b]` **永远是 `[a, b)` 半开区间**——含 a，**不含 b**。所以想把下标 r 这个元素也包进来，stop 必须写 `r + 1`。
...
**为什么要这么设计**：
- 长度直接 `b - a`，不用 ±1 推算
- 空区间自然表示为 `[i, i)`，不会出现 `[i, i-1]` 这种倒置
- 拼接性质：任意 `i`，`s[:i] + s[i:] == s`，永远成立

**推论（高频用法）**：
| 想要拿什么 | 怎么写 |
|---|---|
| 下标 l..r（**闭区间**）的子串 | `s[l : r + 1]` ← 这个 +1 就是为补半开 |
| 前 k 个 | `s[:k]` |
| 从下标 i 起到末尾 | `s[i:]` |
| 不含末尾 1 个 | `s[:-1]` |
| 倒数 k 个 | `s[-k:]` |

**对比 TS 自查**：TS 里 `Array.prototype.slice(a, b)` 和 `String.prototype.substring(a, b)` 也是 `[a, b)` 半开——本质一样。容易掉的是 `String.prototype.substr(start, length)` 给的是长度，与切片不是一回事；写惯 substr 的会写习惯 `+1` 也错。
