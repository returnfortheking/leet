...
### S2. 抄错"主比较轴"——找值类 vs 找边界类模板混用

二分按**最外层比较什么**分两类，模板抄错就卡壳。

| 类型 | 最外层比较 | 代表题 | 内层关心 |
|---|---|---|---|
| **找值类** | `nums[mid] vs target`（命中判断）| **0033** 搜索旋转 / 0035 搜索插入 / 0034 找位置 | 命中后做啥 / 不命中往哪边收 |
| **找边界类** | `nums[mid] vs 端点`（区间判段）| **0153** 旋转数组最小值 / 0162 找峰值 | 哪半符合性质，往那半收 |

**栽点**：把 0153（边界类）的模板套到 0033（找值类）上——**最外层错位**：

```python
# 0153（边界类）的模板：最外层比 mid vs 端点
if nums[mid] >= nums[r]: l = mid + 1
elif nums[mid] < nums[r]: r = mid - 1

# 错搬到 0033：最外层还是 mid vs nums[r]，但题目有 target...
# 结果："命中 target"的逻辑被挤到内层，条件越写越绕
```
...
while l <= r:
    mid = (l + r) // 2
    if nums[mid] == target: return mid        # ★ 命中先判
    if 左半有序:                                # 内层：识别有序半
        if target 在左半: r = mid - 1
        else: l = mid + 1
    else:                                       # 右半有序
        if target 在右半: l = mid + 1
        else: r = mid - 1
```

**心法**：写二分前先**问自己**——

> "**我要不要拿 target 跟 mid 比命中？**"
> - 有 target → 找值类，最外层 `nums[mid] == target`
> - 没 target（只问"最小/峰值/边界"）→ 找边界类，最外层 `nums[mid] vs 端点`

**抄模板前先认题型**，比硬抄代码靠谱。

**错例**：#0033 第一版 — 抄 0153 的"mid vs r"做最外层，逻辑越写越绕，多余的三联条件判 `nums[r] < target and nums[r] < nums[l] and nums[mid] < nums[r]`。复盘后切回"target vs mid 最外层"，把"哪半有序 + target 在哪半"挪到内层，立刻清晰。
...
### C5. 遍历序列重建树：右子树前序起点用"大小"算，别"找位置"

从前序+中序重建树（#0105）：前序结构是 `[根][左子树整块][右子树整块]`。右子树的前序起点 = **根位置 + 1 + 左子树大小**。

```python
left_size = in_p - in_l                       # 中序里根左边的元素个数
root.left  = build(prePos + 1, in_l, in_p - 1)
root.right = build(prePos + 1 + left_size, in_p + 1, in_r)   # ★ 用大小算
```

**错例**：#0105 第一版 — 想"找左子树中序最后一个元素在前序的位置"来定右子树起点：

```python
pre_r = premap[inorder[in_p - 1]]             # ❌ 两个错
root.right = build(pre_r + 1, in_p + 1, in_r)
```

两个错：
1. **概念错**：`inorder[in_p-1]`（左子树中序的最后一个）**在前序里不一定是左子树的最后一个**，间接找位置不可靠。
2. **负索引坑**：根无左子树时 `in_p == in_l`，`in_p - 1` 可能 = -1，`inorder[-1]` 在 Python **不报错，取到末尾元素** → 垃圾值（详见 [pitfalls § 负索引](pitfalls.md)）。

**心法**：**前序定根，中序定左右子树的大小**。用"大小"O(1) 切分前序块，别用"查位置"——大小不会有负索引/概念问题。

**终止条件只留区间判空**：`if in_l > in_r: return None`。别加 `in_l == in_r: return root`（单节点靠正常递归自然处理）或 `prePos >= len`（区间判空已覆盖）——多余且可能引坑。

**触发器**："两种遍历序列重建树" → 前序/后序定根，中序定左右划分。同款 0106（后序+中序）/ 0889（前序+后序）。

---

## 单调栈

### MS1. `while` 条件用 `<` 还是 `<=` —— 决定相等值怎么处理

单调栈的 `while 栈顶 ? 新值` 比较符,**直接决定相等元素是否触发结算**。选错→相邻相等时算错。

| 题目要找 | 比较符 | 相等时 |
|---|---|---|
| 下一个**严格更大** | `arr[stk[-1]] < arr[i]` | **不弹**（相等留栈里继续等真正更大的）|
| 下一个**大于等于** | `arr[stk[-1]] <= arr[i]` | 弹（相等也算"满足"）|

**错例**：#0739 每日温度 — 找"下一个更高温度"应该用 `<`，写成 `<=` 时相邻相等会误结算。

反例 `[89,62,70,58,47,47,46,76,100,70]`，期望 `ans[4]=3`：

```
下标 4 的 47，下标 5 也是 47
用 <= ：下标5来时 47<=47 True → 弹下标4，记 ans[4]=5-4=1   ❌（47→47 不是变暖）
用 <  ：下标5来时 47<47 False → 下标4留栈，等到下标7的76 → ans[4]=7-4=3  ✓
```

**心法**：**题目要"严格" → `<`；题目"含等于也算" → `<=`**。这不是"考没考虑重复值",是"**相等时该不该触发更新**"的语义选择。

**附带教训**：本地 cases 必须含**"相邻相等 + 后面有更大值"**才能区分 `<`/`<=`——否则错写法蒙混过关（#0739 原 cases 漏了这种，靠真测才暴露）。
...
ans = [0] * n
for i in range(n):
    while stack and arr[stack[-1]] < arr[i]:   # 严格找更大（按题调比较符）
        j = stack.pop()
        ans[j] = i - j              # 距离 / 值，看题目
    stack.append(i)
return ans
...
**触发器**:"找下一个/前一个 更大/更小元素" / "找左右第一个比我大/小的" → 单调栈。同族:0739 / 0496 / 0503 / 0084 柱状图 / 0042 接雨水。
