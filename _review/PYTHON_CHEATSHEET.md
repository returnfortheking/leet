...
**适用 list / str / tuple** 等任何序列类型。三参数都可省，省即用默认。

```python
seq[start:stop:step]
       ↑      ↑     ↑
       从哪开始  到哪结束  步长（默认 1，可负）
```
...
arr[2:5]            # → [2, 3, 4]
arr[:3]             # → [0, 1, 2]      省 start → 从头
arr[7:]             # → [7, 8, 9]      省 stop  → 到末尾
arr[:]              # → [0,...,9]      整段（浅拷贝）
arr[2:100]          # → [2,...,9]      越界自动裁剪，不抛

# --- 负下标 ---
arr[-3:]            # → [7, 8, 9]      最后 3 个
arr[:-3]            # → [0,...,6]      去掉最后 3 个
arr[-5:-2]          # → [5, 6, 7]      含 -5，不含 -2

# --- 步长（第 3 参数）---
arr[::2]            # → [0, 2, 4, 6, 8]     每 2 个取 1
arr[1::2]           # → [1, 3, 5, 7, 9]     奇数下标
arr[::-1]           # → [9, 8, ..., 0]      整体反转（★ 最高频）
arr[::-2]           # → [9, 7, 5, 3, 1]     反向每 2 个
arr[5::-1]          # → [5, 4, 3, 2, 1, 0]  从下标 5 反着到头
arr[8:2:-1]         # → [8, 7, 6, 5, 4, 3]  闭区间到下一位
...
**坑：**
- 切片永远 `[start, stop)` **左闭右开**（同 `range`）—— 想含 `r` 索引必须 `+1`
- step 为负时，省略 `start` 默认 `len-1`，省略 `stop` 默认 `-len-1`（走到头）
- **永远返回新对象**，不修改原序列
- 越界不抛——`arr[100:200]` 返回 `[]`
- 字符串切片返回 str；list 切片返回 list；tuple 切片返回 tuple——**类型保持**
...
**模式**：
- **大顶堆** → 存 `-x`，弹时 `-` 还原（进出都取负；手动 push 写 `heappush(h, -x)`，整体建堆写 `[-x for x in nums]` 再 heapify）
- **元组比较** → 按字典序：`heappush(h, (-freq, item))` 主键取负降序 + 次键升序
- **TopK** → 维护**大小为 k 的堆**，堆顶守门槛

### TopK 心法（反直觉，必背）

| 要找 | 用什么堆 | 堆顶 = | 淘汰谁 |
|---|---|---|---|
| **前 K 大 / 第 K 大** | **小顶堆**（正值）| 这 K 个里最小 = 门槛 | 比门槛小的进不来 |
| **前 K 小 / 第 K 小** | **大顶堆**（取负）| 这 K 个里最大 = 门槛 | 比门槛大的进不来 |

```python
# 第 K 大：小顶堆维护大小 k，堆顶就是第 K 大
h = []
for x in nums:
    heapq.heappush(h, x)
    if len(h) > k:
        heapq.heappop(h)        # 超 k 就弹最小 → 留下最大的 k 个
return h[0]                     # 堆顶 = 第 K 大
```

**为什么找大反而用小顶堆**：你要淘汰"不够大的"，得 O(1) 拿到当前 k 个里最小的去 PK——小顶堆堆顶正好是最小。**堆的方向跟你要找的相反**。复杂度 O(n log k)，优于排序 O(n log n)，且适合数据流（来一个处理一个）。
